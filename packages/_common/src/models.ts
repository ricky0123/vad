...
export class Silero {
  ...
  static new = async (ort: ONNXRuntimeAPI, modelFetcher: ModelFetcher, sampleRate: number) => {
    const model = new Silero(ort, modelFetcher, sampleRate)
    await model.init()
    return model
  }
  ...
  init = async () => {
    ...
    const tensorSize = BigInt(this.sampleRate)
